...
function VM (props) {
    ...
    const handleUpdateSubmit = async (e)=>{
        ...
        if (!form.checkValidity()) {
            ...
            tableEntry["name"] = json['name']
            tableEntry["zone"] = json['zone_name']
            tableEntry["eIP"] = json['external_ip']
            tableEntry["status"] = false
            json['status'] = 'Stopped'
            json['internal_ip'] = updateVM.internal_ip
            tableEntry['iIP'] = updateVM.internal_ip
            json['ram']= parseInt(json['ram'])
            let date = new Date();
            date = date.toISOString();
            json['date'] = `${date.slice(0,10)} ${date.slice(11,19)}`
            json['userID'] = props.location.state.user
            json['vmID'] = updateVM.vm_id
            json['projectID'] = proj;
            console.log(json)
            axios({method:"POST",
            url:"http://localhost:8008/vm/edit_config",
            data: json        
        })
        .then((res)=>{
            if(res.data.vm == 1){
    ...
    // nvm,gpuP,gpu,disk,mfP,mf
        // quotas = [{nvm:5, disk:[2,3,4], gpuP:[1,1,1,1,1], gpu:[1,1,1,1,1],mfP:[2,2,2,2,2], mf:[0,0,2,2,2]},
        ...
        const handleUpdateVM=async (e)=>{
            ...
            await axios({method:"POST",
                ...
                .then((res)=>{
                    ...
                    if(res.data.preemptibility === null)
                        res.data.preemptibility = false;

                    res.data.gpu = res.data.gpu.replace(/[()]/g,'').split(',').indexOf('1')+1
                    res.data.disk = res.data.disk.replace(/[()]/g,'').split(',')
                    res.data.diskLoc = res.data.disk.findIndex((ele)=>ele!=0)+1
                    res.data.disk = parseInt(res.data.disk[res.data.disk.findIndex((ele)=>ele!=0)])
                    console.log(res.data)
                    setUpdateVM(res.data);
                    setRamValue(res.data.ram);
                    setDiskValue(res.data.disk);
                })
                .catch((err)=>console.log(err))
        }
                ...
                <Container class="rounded border border-dark text-center w-50">
                    ...
                    <Row>
                        <Container className="rounded border border-light text-center d-flex justify-content-around w-75 p-3">
                            ...
                            <Button variant="primary" size='md' onClick={() => {setTransferShow(true);projQuota()}}>Update VM</Button>
                            ...
                            <Modal size="lg" show={transferShow} ref={updateRef} onHide={() => {setTransferShow(false);updateRef.current.reset();setIpShow(false)}} aria-labelledby="contained-modal-title-vcenter" centered backdrop="static" keyboard={false}>
                                ...
                                <Modal.Body>
                                <Form ref={updateRef} id="update" noValidate validated={validated} onSubmit={handleUpdateSubmit}>
                                        <Container centered>
                                            <Row>
                                                <Form.Group controlId="updateVm">
                                                    ...
                                                    {vmdata.map((row)=> {return(<option id={row.vm_id}>{row.name}</option>)})}
                                                    </Form.Control>
                                                </Form.Group>
                                            </Row>
                                            <br/>
                                        ...
                                        <Col>
                                            <Form.Group controlId="formName">
                                                ...
                                                <Form.Control type="text" value={updateVM.name+' ('+updateVM.vm_id+')'} disabled/>
                                            </Form.Group>
                                        </Col>
                                        </Row>
                                        <br/>
                                        ...
                                        <Col>
                                            <Form.Group controlId="formZone">
                                                <Form.Label>Zone</Form.Label>
                                                <Form.Control as="select" name="zone_name" value={updateVM.zone_name} onChange={zoneChange}>
                                                    {/* <option>Select Zone</option> */}
                                                    <option>us-central-a</option>
                                                    <option>us-central-b</option>
                                                    <option>eu-east-a</option>
                                        ...
                                        <br/><br/>
                                            <Form.Group controlId="formPreempt">
                                                <Form.Check type="checkbox" id="preempt" checked={updateVM.preemptibility} name="preemptibility" label="Pre-emptibility" onClick={()=>setPreempt(!preempt)}/>
                                            </Form.Group>
                                        ...
                                        <Col>
                                            <Form.Group controlId="formType">
                                                <Form.Label>Machine Type</Form.Label>
                                                <Form.Control as="select" name="machine" value={updateVM.machine} required onChange={machineChange}>
                                                {/* <option>None</option> */}
                                                {/* {console.log(preempt?credits["mfP"][0]:credits["mf"][0])} */}
                                                <option disabled={(preempt?quotas[zone].mfP[0]:quotas[zone].mf[0])>0?false:true} value='EC2'>EC2 (Available {preempt?quotas[zone].mfP[0]:quotas[zone].mf[0]} Machines)</option>
                                                <option disabled={(preempt?quotas[zone].mfP[1]:quotas[zone].mf[1])>0?false:true} value='N1'>N1 (Available {preempt?quotas[zone].mfP[1]:quotas[zone].mf[1]} Machines)</option>
                                                <option disabled={(preempt?quotas[zone].mfP[2]:quotas[zone].mf[2])>0?false:true} value='N2'>N2 (Available {preempt?quotas[zone].mfP[2]:quotas[zone].mf[2]} Machines)</option>
                                                <option disabled={(preempt?quotas[zone].mfP[3]:quotas[zone].mf[3])>0?false:true} value='C2'>C2 (Available {preempt?quotas[zone].mfP[3]:quotas[zone].mf[3]} Machines)</option>
                                                <option disabled={(preempt?quotas[zone].mfP[4]:quotas[zone].mf[4])>0?false:true} value='A2'>A2 (Available {preempt?quotas[zone].mfP[4]:quotas[zone].mf[4]} Machines)</option>
                                                </Form.Control>
                                            </Form.Group>
                                        </Col>
                                        <Col>
                                            <Form.Group controlId="formGPU">
                                                <Form.Label>GPU</Form.Label>
                                                <Form.Control as="select" name="gpu" value={updateVM.gpu} required onChange={gpuChange}>
                                                <option>None</option>
                                                <option disabled={((!isA100 ? false : true) && (preempt?quotas[zone].gpuP[0]:quotas[zone].gpu[0])>0 ? false:true)} value='1'>NVIDIA_TESLA_A100 (Available {preempt?quotas[zone].gpuP[0]:quotas[zone].gpu[0]} GPU)</option>
                                                <option disabled={((!isV100 ? false : true) && (preempt?quotas[zone].gpuP[1]:quotas[zone].gpu[1])>0 ? false:true)} value='2'>NVIDIA_TESLA_V100 (Available {preempt?quotas[zone].gpuP[1]:quotas[zone].gpu[1]} GPU)</option>
                                                <option disabled={((!isK80 ? false : true)  && (preempt?quotas[zone].gpuP[2]:quotas[zone].gpu[2])>0 ? false:true)} value='3'>NVIDIA_TESLA_K80 (Available {preempt?quotas[zone].gpuP[2]:quotas[zone].gpu[2]} GPU)</option>
                                                <option disabled={((!isT4 ? false : true)   && (preempt?quotas[zone].gpuP[3]:quotas[zone].gpu[3])>0 ? false:true)} value='4'>NVIDIA_TESLA_T4 (Available {preempt?quotas[zone].gpuP[3]:quotas[zone].gpu[3]} GPU)</option>
                                                <option disabled={((!isP4 ? false : true)   && (preempt?quotas[zone].gpuP[4]:quotas[zone].gpu[4])>0 ? false:true)} value='5'>NVIDIA_TESLA_P4 (Available {preempt?quotas[zone].gpuP[4]:quotas[zone].gpu[4]} GPU)</option>
                                                </Form.Control>
                                            </Form.Group>
                                        </Col>
                                        </Row>
                                        ...
                                        <Col>
                                            <Form.Group controlId="formDisk">
                                                <Form.Label>Disk</Form.Label>
                                                <Form.Control as="select" name="disk" value={updateVM.diskLoc} onChange={diskChange} required>
                                                {/* <option>Select Disk Type</option> */}
                                                <option disabled={quotas[zone].disk[0]>0?false : true} value='1'>SSD (Available {quotas[zone].disk[0]} PB)</option>
                                                <option disabled={quotas[zone].disk[1]>0?false : true} value='2'>HDD (Available {quotas[zone].disk[1]} PB)</option>
                                                <option disabled={quotas[zone].disk[2]>0?false : true} value='3'>Balanced (Available {quotas[zone].disk[2]} PB)</option>
                                                </Form.Control>
                                            </Form.Group>
                                        </Col>
                                        </Row>
                                        ...
                                        <Col>
                                            <Form.Group controlId="formDisk">
                                                <Form.Label>Boot_Disk Image</Form.Label>
                                                <Form.Control as="select" value={updateVM.boot_disk} name="boot_disk" required>
                                                {/* <option>Select Image</option> */}
                                                {diskImage.map((row,index)=> {return(<option value={row.name}>{row.name}</option>)})}
                                                </Form.Control>
                                            </Form.Group>
                                        </Col>
                                        </Row>
                                        ...
                                        <Col>
                                            <Form.Group controlId="formExternalIP">
                                            <Form.Control type='text' value={updateVM.external_ip} name="external_ip"/>
                                            </Form.Group>
                                        </Col>
                                        </Row>
                                        <br/>
                                        <Row>
                                            <Col>
                                                <Form.Group controlId="formNetworkTag">
                                                    <Form.Label>Network Tag</Form.Label>
                                                    <Form.Control type='text' name="network_tag" value={updateVM.network_tag} onClick={()=>setNetCost(credits["network"])}/>
                                                </Form.Group>
                                            </Col>
                                            <Col>
                                                <Form.Group controlId="formHostName">
                                                    <Form.Label>Host Name</Form.Label>
                                                    <Form.Control type='text' value={updateVM.host_name} name="host_name"/>
                                                </Form.Group>
                                            </Col>
                                        </Row>
                                        <br/>
